...
bool cfgUseBikeLogicOnQuads = false;
...
public:
    RealTrafficFix()
	{
		...
		cfgUseBikeLogicOnQuads		 = ReadIniBool(ini, &lg, "Settings", "UseBikeLogicOnQuads");
		...
		if (cfgSlowVehiclesSlowLane) {
			...
			{
				CVehicle *veh = (CVehicle *)regs.esi;
				if ((veh->bIsBig || veh->bIsBus || veh->m_pHandlingData->m_transmissionData.m_fMaxGearVelocity < 0.62) &&
					(!veh->bIsLawEnforcer && !veh->bIsAmbulanceOnDuty && !veh->bIsFireTruckOnDuty))
				{
					if (regs.ecx > 1)
					{
		...
		*/
			...
			if (vehicle->m_pDriver && vehicle->m_nCreatedBy != eVehicleCreatedBy::MISSION_VEHICLE)
			{
				if ((!vehicle->m_pDriver->IsPlayer() && (!cfgTestMode || !playerAutoDrive)) || (cfgTestMode || playerAutoDrive)) //wtf
				{
						...
						subClass == VEHICLE_BIKE)
					{
						...
						bool isBike = false;
					#if defined(GTASA)
						if (subClass == VEHICLE_BIKE || subClass == VEHICLE_BMX || subClass == VEHICLE_QUAD) { isBike = true; }
					#else
					...
					*/
					...
					// Is valid driving
						...
						if (vehicle->m_autoPilot.m_pTargetCar == nullptr &&
							(!vehicle->bSirenOrAlarm || vehicle->m_nModelIndex == MODEL_MRWHOOP))
					#else
						...
						if (vehicle->m_autoPilot.m_nCarDrivingStyle <= 2 || vehicle->m_autoPilot.m_nCarDrivingStyle == 4) // ok to change
						{
							...
							else
							{
								...
								if (vehicle->m_nVehicleSubClass == VEHICLE_QUAD)
								{
									if (cfgUseBikeLogicOnQuads)
									{
										vehicle->m_autoPilot.m_nCarDrivingStyle = (eCarDrivingStyle)6;
									}
								}
						...
						label_force_no_obstacle:
							...
							else
							{
								...
								if (vehicle->m_fGasPedal >= 0.05f && !vehicle->bParking)
							#else
		...
		{
			((RwGlobals*)RwEngineInstance)->dOpenDevice.fpRenderStateSet(rwRENDERSTATEZTESTENABLE, FALSE);
			...
			((RwGlobals*)RwEngineInstance)->dOpenDevice.fpRenderStateSet(rwRENDERSTATEZTESTENABLE, (void*)TRUE);
